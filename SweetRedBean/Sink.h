...
/*
Sink는 Pass의 입력에 대한 연결을 표현한 클래스이다.
*/
...
#pragma region DirectBufferSinkFunc
	...
	template<class T>
	void DirectBufferSink<T>::Bind(Source& source)
	{
...
#pragma region ContainerBindableSinkFunc
	...
	template<class T>
	void ContainerBindableSink<T>::Bind(Source& source)
	{
...
#pragma region DirectBindableSinkFunc
	...
	template<class T>
	void DirectBindableSink<T>::Bind(Source& source)
	{
		auto p = std::dynamic_pointer_cast<T>(source.YieldBindable());
		assert(p != nullptr);
		mTarget = std::move(p);
