...
bool RenderManager::Init(HWND hwnd, HINSTANCE hInstance, UINT width, UINT height)
{
	Release();
...
void RenderManager::BeginRender()
{
	...
	for (size_t i = 0; i < OFFSCREEN_SIZE; ++i)
	{
		mRenderTargets[i].ClearRenderTarget(common::Black);
	}
...
{
	// 오브젝트들이 전역적으로 받아야할 상수 버퍼는 여기서 업데이트 한다.
