...
public:
		...
		, mDepthMapSRV(nullptr)
	{
        ...
        texDesc.MipLevels = 1;
        texDesc.ArraySize = 1;
        texDesc.Format = DXGI_FORMAT_R24G8_TYPELESS; // typeless로 두고 뷰에서 해석
        ...
        dsvDesc.Flags = 0;
        dsvDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT; // DSV에서는 스텐실 판정이 쓰이므로 요놈
        dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
        dsvDesc.Texture2D.MipSlice = 0;
        HR(device->CreateDepthStencilView(depthMap, &dsvDesc, &mDepthMapDSV));

        D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
        srvDesc.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS; // SRV에서는 스텐실은 안 쓰므로 typeless
